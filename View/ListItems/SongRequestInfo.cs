...
namespace View.ListItems
{
    public class SongRequestInfo
    {
        ...
        public SongRequestInfo(Request request)
        {
            ...
            Request = request;
            Song = SongController.Create(new DatabaseContext()).GetItem(request.SongID.Value);
            ArtistName = ArtistController.Create(new DatabaseContext()).GetItem(Song.Artist).ArtistName;
