...
namespace Tests.SearchTests
{
    ...
    public class Controller_Song_SearchOnString
    {
        ...
        public void SetUp()
        {
            ...
            songController.CreateItem(new Song() 
                { ID = 1, Name = "SongNameTest", ArtistID = 1, Duration = 10, Path = "../path" });
            songController.CreateItem(new Song() 
                { ID = 2, Name = "Ik WIl Testen, liefst ieder kwartier", ArtistID = 1, Duration = 10, Path = "../path" });
        }
        ...
        {
            songController
                .SearchSongsOnString(new List<string>() {search})
                .ContinueWith(res => 
                Assert.IsTrue(res.Result.Any(x => x.ID == expectedId))
            );
        }
        ...
        {
            songController.SearchSongsOnString(searches.ToList()).ContinueWith(res => 
                Assert.IsFalse(res.Result.Any(x => !expected.Contains(x.ID)))
            );
        }
